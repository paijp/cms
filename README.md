...
（既定パス `/var/www/kimoken-cms/site.php`、環境変数 `CMS_CONFIG` で変更可）。
...
- 記事データ（`data_dir` / `drafts_dir` の親ディレクトリ）と管理者トークン
- `site.php`（リポジトリに含まれないため）
- nginx のサイト設定
- `sessions/` は不要（消えてもトークンURLで再ログインできる）
